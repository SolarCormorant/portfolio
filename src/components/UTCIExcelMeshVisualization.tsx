...
const UTCIExcelMeshVisualization = ({
  ...
  const [contextMeshData, setContextMeshData] = useState(null); // <<— YENİ
  ...
  const parseObjData = useCallback((objContent) => {
    ...
    for (const raw of lines) {
      ...
      if (parts[0] === 'v') {
        ...
        const faceIndices = parts.slice(1).map(idx => parseInt(idx.split('/')[0]) - 1);
  ...
  const renderMesh = useCallback(() => {
    ...
    // <<— YENİ: Context (tek renk) mesh
    const contextTrace = contextMeshData && {
      type: 'mesh3d',
      x: contextMeshData.vertices.map(v => v[0]),
      y: contextMeshData.vertices.map(v => v[1]),
      z: contextMeshData.vertices.map(v => v[2]),
      i: contextMeshData.faces.map(f => f[0]),
      j: contextMeshData.faces.map(f => f[1]),
      k: contextMeshData.faces.map(f => f[2]),
      intensitymode: 'none',     // tek renk
      color: contextColor,       // tek renk
      opacity: contextOpacity,   // şeffaflık
      showscale: false,
      flatshading: true,
    ...
    // Context varsa önce onu, sonra ana mesh’i çiz (arka planda kalsın)
  ...
  useEffect(() => {
    const loadFiles = async () => {
      ...
      try {
        ...
        // <<— YENİ: Context OBJ yükleme (varsa)
    ...
    excelFilePath,
    objFilePath,
    contextObjFilePath, // <<— YENİ: context yolu değişirse yeniden yükle
    loadExcelData,
    loadObjData,
    parseObjData
  ]);
